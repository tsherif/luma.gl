...
export default class VertexArray extends Resource {
  ...
  setAttributes(attributes) {
    this.bind(() => {
      for (const locationOrName in attributes) {
        const attribute = attributes[locationOrName];

        // TODO - should we support descriptors here? Leave this to Model to resolve?
        const value = attribute.buffer || attribute.externalBuffer || attribute;

        if (value instanceof Buffer) {
          this.setBuffer(locationOrName, value, attribute);
        } else if (ArrayBuffer.isView(value)) {
          this.setConstant(locationOrName, value, attribute);
        } else {
  ...
  _getDebugTable({header = 'Attributes'} = {}) {
    ...
    for (const attributeName in attributes) {
      ...
      if (info) {
        ...
        const rowHeader = `${attributeName}: ${info.name}`;
        table[rowHeader] =
  ...
  _getDebugTableRow(attribute, accessor, header) {
    ...
    if (attribute instanceof Buffer) {
      ...
      value = buffer.data || (isWebGL2(this.gl) && buffer.getData({length: 24})) || 'N/A';
      bytes = buffer.bytes;
  ...
  // DEPRECATED/REMOVED in v6.0

  setBuffers(buffers) {
    log.deprecated('VertexArray.setBuffers', 'VertexArray.setAttributes', 'v6.0')();
    return this.setAttributes(buffers);
  }

  // Specify values for generic vertex attributes
  setGeneric({location, array}) {
    log.deprecated('VertexArray.setGeneric', 'VertexArray.setConstant', 'v6.0')();
    return this.setConstant(location, array);
  }
  ...
  checkAttributeBindings() {
    /*
    const filledLocations = this.filledLocations;
